...
public class GtdDeedTodoRecyclerView extends ABaseLinearRecyclerView<GtdDeedEntity> {
    private GtdLifeCycleAction gtdActLifeCycleAction;

    public GtdDeedTodoRecyclerView(Context context) {
        super(context);
        gtdActLifeCycleAction = new GtdLifeCycleAction();
    }

    public GtdDeedTodoRecyclerView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        gtdActLifeCycleAction = new GtdLifeCycleAction();
    }

    public GtdDeedTodoRecyclerView(Context context, @Nullable AttributeSet attrs, int defStyle) {
    ...
    public int getViewItemLayout() {
        return R.layout.view_recycler_deed_todo_item;
    }
    ...
    class ActionItemAdapter extends BaseItemAdapter<GtdDeedEntity>{
        ...
        protected void convert(BaseViewHolder helper, GtdDeedEntity item) {
            ...
            helper.setText(R.id.view_recycler_deed_todo_item_title, titleBuilder.toString())
                .setText(R.id.view_recycler_deed_todo_item_detail, detailBuilder.toString())
                .addOnClickListener(R.id.view_recycler_deed_todo_item_btn_done)
                .setImageDrawable(R.id.view_recycler_deed_todo_item_image, TextImageFactory.getInstance().getGtdActionImage());
        }
    }
}
