 ...
 */
public class DeedDoneListMvpP implements IAllItemListMvpP<GtdDeedEntity> {
    private IGtdTodoActionListMvpV mvpV;
    private EntityListMvpM mvpM;

    private List<GtdDeedEntity> entityList;

    public DeedDoneListMvpP(IGtdTodoActionListMvpV mvpV) {
        this.mvpV   = mvpV;
    ...
    class EntityListMvpM{
        public Flowable<GtdDeedEntity> loadAllEntity() {
            return GtdDeedByUuidFactory.getInstance().getEntityByState(DeedState.DONE)
                    .sorted(new DeedWarningTimeComparator().getDescOrder());
        }
    }
